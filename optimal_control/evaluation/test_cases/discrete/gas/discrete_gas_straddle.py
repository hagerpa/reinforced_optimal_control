...
from optimal_control.examples.discrete.gas.discrete_gas_straddle import DiscreteGasStraddle
...
class DiscreteGasStraddleTestCase(TestCase):
    ...
    def test_environment(T: float, J: int,
        ...
        example = DiscreteGasStraddle(T=T,
                                      n_time_steps=J + 1,
    ...
    def value_function(test_environment, deg=0, max_depth=None):
        ...
        def reinforced_value_functions(y: int):
            return np.array([y])
            # return np.arange(max(0, y - 2), y)
            # return np.arange(y, min(y, test_environment.example.y_max) + 1)
            # return np.arange(0, 9, 3) #works
            # return np.arange(max(0, y-4), min(y+4, test_environment.example.y_max)+1)
            # return np.array(np.arange(9)) # works
