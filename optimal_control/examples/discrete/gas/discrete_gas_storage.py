...
class DiscreteGasStorage(SwingOption):
    ...
    def payoff(self, t: int, H, Y, X):
        if t == 0 or t == self.n_time_steps - 1:
            ...
            h = H[:, 0]
            p = h * X[:, 0] * np.exp(-self.interest_rate * t * self.time_delta)
            if self.log_utility:
                log_utility_f = lambda p_: np.sign(p_) * np.log(1 + np.abs(p_))  # sgn(x)log(1 +|x|)
